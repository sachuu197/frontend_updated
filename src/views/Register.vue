        ...
        <div v-if="!successful">
          ...
          <select name="category_id" @change="onChange12($event)" v-model="user.college" class="form-control">
             <option>--- Select College ---</option>
               <option v-for="(item , index) in colleges" v-bind:key="index" :value="item.college_name" >
                {{item.college_name}}
             </option>
         </select>
...
export default {
  ...
  data() {
    return {
      user: new User('', '', ''),
      submitted: false,
     colleges:[{"id":"1","college_name":"VIT"},{"id":"2","college_name":"VIIT"},{"id":"3","college_name":"VU"}],
      successful: false,
      message: '',
      collges:''
  ...
  /*  getTasks() {
        axios.get(`http://localhost:3000/college/${this.id}`).then(
          result => {
